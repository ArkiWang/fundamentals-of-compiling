...
public class StaticCheck {
    ...
    private void declareStatement(){
        if(lookAhead.getType()==TokenType.KEY_INT){
            matchToken(TokenType.KEY_INT,"declareStatement");
            idList(TokenType.KEY_INT);
            matchToken(TokenType.SEMICOLON,"declareStatement");
        }else if(lookAhead.getType()==TokenType.KEY_BOOLEAN){
            matchToken(TokenType.KEY_BOOLEAN,"declareStatement");
            idList(TokenType.KEY_BOOLEAN);
            matchToken(TokenType.SEMICOLON,"declareStatement");
        }else {
            String errorTypes=TokenType.KEY_INT.toString()
                    +","+TokenType.KEY_BOOLEAN.toString();
            RecursionDescendParser.parsingError(errorTypes,"declareStatement",lookAhead);
        }
    }
    private void idList(TokenType type){
        if (lookAhead.getType()==TokenType.IDENTIFIER){
            Token id=matchToken(TokenType.IDENTIFIER,"idList");
            symTable.insert(id.getLexeme(),type,this.blockLevel);
            idListN(type);
        }else {
            String errorTypes=TokenType.IDENTIFIER.toString();
            RecursionDescendParser.parsingError(errorTypes,"idList",lookAhead);
        }
    }
    private void idListN(TokenType type){
        if(lookAhead.getType()==TokenType.COMMA){
            matchToken(TokenType.COMMA,"idListN");
            Token id=matchToken(TokenType.IDENTIFIER,"idListN");
            symTable.insert(id.getLexeme(),type,this.blockLevel);
            idListN(type);
            ...
            String errorTypes=TokenType.COMMA.toString()
                    +","+TokenType.SEMICOLON.toString();
            RecursionDescendParser.parsingError(errorTypes,"idListN",lookAhead);
        }
    }
    ...
    private ExpressionStatement assignmentStatement(){
        if (lookAhead.getType()== TokenType.IDENTIFIER){
            ...
            /*检查标识符是否已经在符号表中定义了*/
            SymTable.Symbol symbol=symTable.lookup(id.getLexeme(),this.blockLevel);
            if(symbol==null){
                System.out.println("static check error!");
                System.out.println("found undefined variable:"+id.getLexeme());
                System.out.println("at line="+id.getLine()+",column="+id.getColumn());
                System.exit(1);
            }
